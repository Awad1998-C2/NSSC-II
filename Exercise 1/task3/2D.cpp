...
int main(int argc, char** argv) {
    ...
    auto init = [num1, num2, coord, width, height, W = opts.fix_west, E = opts.fix_east]() -> auto {
        std::vector<double> res(static_cast<size_t>(width) * static_cast<size_t>(height));
        for (size_t j = 0; j < static_cast<size_t>(height); ++j)
            for (size_t i = 0; i < static_cast<size_t>(width); ++i) {
                res[i + j * static_cast<size_t>(width)] = 0.0;
                if (i % static_cast<size_t>(width) == 0)
                    res[i + j * static_cast<size_t>(width)] = (coord[1] == 0) ? W : 0;
                if (i % static_cast<size_t>(width) == static_cast<size_t>(width) - 1)
                    res[i + j * static_cast<size_t>(width)] = (coord[1] == num2 - 1) ? E : 0;
            }
        return res;
    };
    ...
    auto jacobi_iter = [width, height, coord, num1, num2, N=opts.N](const auto &xold, auto &xnew,
        ...
        if (coord[0]==0) {
            ...
            // all interior points
            for (size_t j = 1; j < static_cast<size_t>(height-1); ++j) {
                for (size_t i = 1; i < static_cast<size_t>(width - 1); ++i) {
                    auto w = xold[(i - 1) + (j)*width];
                    auto e = xold[(i + 1) + (j)*width];
                    auto n = xold[(i) + (j + 1) * width];
    ...
    // Helper function to perform MPI_Gather
    auto perform_gather = [](const auto& vec, int start_idx, int count, MPI_Datatype datatype,
                         std::vector<double>& recvbuf, int recv_offset, MPI_Comm comm) {
    MPI_Gather(&vec[start_idx], count, datatype,
               std::data(recvbuf) + recv_offset, count, datatype, 0, comm);
    };

    // Improved gather_all_parts_vertical
    auto gather_all_parts_vertical = [N = opts.N, coord, height, width, col_comm, row, num1, &perform_gather](const auto& vec) -> auto {
    std::vector<double> recvbuf;

    // Initialize receive buffer only for processes with column rank 0
    if (coord[0] == 0) {
        recvbuf.resize(N * width, -1);
    }

    // Determine the data to gather based on the process's position
    int start_idx = (coord[0] == 0) ? (N % num1) * width : width;
    int count = (coord[0] == 0) ? height - 1 - (N % num1) : (coord[0] == num1 - 1) ? height - 1 : height - 2;

    // Perform the gather operation
    perform_gather(vec, start_idx, count, row, recvbuf, (N % num1) * width, col_comm);

    return recvbuf;
    };

    // Improved gather_all_parts_horizontal
    auto gather_all_parts_horizontal = [N = opts.N, coord, width, row_comm, long_row, num2, &perform_gather](const auto& vec) -> auto {
    std::vector<double> recvbuf;

    // Initialize receive buffer only for process (0, 0)
    if (coord[0] == 0 && coord[1] == 0) {
        recvbuf.resize(N * N, -1);
    }

    // Determine the data to gather based on the process's position
    int start_idx = (coord[1] == 0) ? (N % num2) * N : N;
    int count = (coord[1] == 0) ? width - 1 - (N % num2) : (coord[1] == num2 - 1) ? width - 1 : width - 2;

    // Perform the gather operation
    perform_gather(vec, start_idx, count, long_row, recvbuf, (N % num2) * N, row_comm);
